document.getElementById("nilaiPerolehanTanahForm").addEventListener("submit", function (event) {
    ...
    var hitungType = document.querySelector('input[name="hitungType"]:checked').value;

    // Mendefinisikan variabel numerik lainnya
    var njop = parseFloat(document.getElementById("njop").value);
    var persenNjop = parseFloat(document.getElementById("persenNjop").value) / 100;
    ...
    var bphtb, pph, kasDesa, biayaNotaris, admin, rtrw, validasi, paralegal, sppt, segelJualBeli, nilaiPerkiraanHarga;

    // Menghitung berdasarkan pilihan hitung
    if (hitungType === "hitung-ajb") {
        // Menghitung bphtb
        bphtb = nilaiPerolehanTanah >= 80000000 ? (nilaiPerolehanTanah - 80000000) * 0.05 : 0;
        // Menghitung pph
        ...
        nilaiPerkiraanHarga = bphtb + pph + kasDesa + biayaNotaris + admin + rtrw + validasi + paralegal + sppt;
    ...
    hasilElem.innerHTML = `
        <br><h2>Rencana Anggaran Biaya Pembuatan Surat Tanah</h2><br> 
